...
nbrshared_allmth <- do.call(rbind, lapply(all_nsamples, function(ss) {
  do.call(rbind, lapply(all_repl, function(i) {
    tmp <- pconc[, intersect(which(get_repl(colnames(pconc)) == i),
                             which(get_nsamples(colnames(pconc)) == ss)), drop = FALSE]
    if (ncol(tmp) > 1) {
      concval <- calculate_nbr_occurrences(mtx = tmp, maxrank = maxrank)
      concval$nbr_methods <- ncol(tmp)
      concval$ncells <- ss
      concval$repl <- i
      concval
    } else {
...
summary_data$nbrshared_allmethods <- 
  rbind(summary_data$nbrshared_allmethods, 
        nbrshared_allmth %>% dplyr::mutate(dataset = dataset, filt = filt))
