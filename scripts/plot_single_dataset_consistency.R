...
plot_consistency <- function(cobra, concordances, colvec, exts, summary_data = list()) {
  ...
  cmcons <- concordances$concordance_betweenmethods_auc %>% dplyr::group_by(method1, method2) %>%
  ...
  ggplot(cmdist, aes(x = auc)) + geom_line(stat = "density") +
    ...
    xlim(0, 1) + xlab("AUC")
  ggplot(cmdist, aes(x = auc)) + geom_histogram() +
    facet_grid(method1 ~ method2, scales = "free") + theme_bw() +
    theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
          axis.ticks = element_blank(), strip.text.x = element_text(size = 6, angle = 90),
          strip.text.y = element_text(size = 6, angle = 0), panel.grid = element_blank()) +
  ...
  ggplot(cmdist2, aes(x = ordr, y = ycoord)) + geom_raster(aes(fill = auc)) +
  ...
  ggplot(cmdist2, aes(x = ncells, y = auc)) + geom_point(size = 0.5) +
    geom_smooth() +
    facet_grid(method1 ~ method2) + theme_bw() +
    theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
          axis.ticks = element_blank(), strip.text.x = element_text(size = 6, angle = 90),
